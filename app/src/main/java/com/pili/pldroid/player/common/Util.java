...
public final class Util {
    ...
    public static String getUserAgent(Context var0, String var1) {
        ...
        return var1 + "/" + var2 + " (Linux;Android " + VERSION.RELEASE + ") " + "PLDroidPlayer/" + "1.5.0";
    }

    public static Pair<Integer, Integer> getResolution(Context var0) {
        return VERSION.SDK_INT >= 17?getRealResolution(var0):getRealResolutionOnOldDevice(var0);
    }
    ...
    public static int[] computeRowAndColumn(int size) {
        int[] grid = new int[2];
        int sqrt = (int)Math.sqrt(size);
        if(sqrt * sqrt < size){
            grid[0] = sqrt + 1;
            if(size%grid[0] > 0) {
                grid[1] = size/grid[0] + 1;
            } else {
                grid[1] = size/grid[0];
            }
        } else {
