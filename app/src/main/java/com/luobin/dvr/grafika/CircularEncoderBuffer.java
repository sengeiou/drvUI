...
public class CircularEncoderBuffer {
    ...
    static public abstract class Listener {
        public abstract void onBufferAdded();
    };
    ...
    public CircularEncoderBuffer(int bitRate, int frameRate, int desiredSpanSec) {
        ...
        int dataBufferSize = (int) ((bitRate * desiredSpanSec * 1.4) / 8);
        ...
        int metaBufferCount = (int) (frameRate * desiredSpanSec * 1.4 * 2);
        ...
        mPacketTrackId = new int[metaBufferCount];
     ...
     */
    public void add(ByteBuffer buf, int track, int flags, long ptsUsec) {
        int size = buf.limit() - buf.position();
        if (VERBOSE) {
            Log.d(TAG, "add track=" + track + " size=" + size + " flags=0x" + Integer.toHexString(flags) +
                    " pts=" + ptsUsec);
        }
        while (!canAdd(size)) {
            removeTail();
        ...
        try {
            ...
            mPacketTrackId[mMetaHead] = track;
            ...
            if (VERBOSE) {
                Log.v(TAG, "mMetaHead = " + mMetaHead + " after add()");
            }
    ...
    public int getFirstIndex() {
        ...
        int index = mMetaTail;
        Log.v(TAG, "getFirstIndex metaLen = " + metaLen + " mMetaHead = " + mMetaHead
                +" mMetaTail = " + mMetaTail);
    ...
    public int getCurrentIndex() {
        final int metaLen = mPacketStart.length;
        Log.d(TAG,"getCurrentIndex mMetaHead=" + mMetaHead);
        if (mMetaHead == mMetaTail) {
            // empty list
            Log.d(TAG,"getCurrentIndex empty");
            return -1;
        }
        return (mMetaHead + metaLen -1) % metaLen;
    }

    public int getLastIndex(int lastflag) {
        final int metaLen = mPacketStart.length;

        //int index = (mMetaHead + metaLen -1) % metaLen;
        int index = lastflag < 0 ? 0 : lastflag;
        Log.v(TAG, "getLastIndex lastflag = "+lastflag+ " metaLen = " + metaLen + " mMetaHead = " + mMetaHead
            +" mMetaTail = " + mMetaTail);

        while (index != ((mMetaTail + metaLen -1) % metaLen)) {
            if ((mPacketFlags[index] & MediaCodec.BUFFER_FLAG_SYNC_FRAME) != 0) {
                break;
            }
        ...
        if (VERBOSE) {
            Log.v(TAG, "getLastIndex index = " + index);
        }
        if (index == ((mMetaTail + metaLen -1) % metaLen)) {
            Log.w(TAG, "HEY: could not find sync frame in buffer");
            index = -1;
        }
        return index;
    ...
    public int getNextIndex(int index) {
        ...
        if (VERBOSE) {
            Log.v(TAG, "getNextIndex index = " + index + " metaLen = " + metaLen + "next = " + next);
        }
        return next;
    }
