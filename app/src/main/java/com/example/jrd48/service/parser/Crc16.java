package com.example.jrd48.service.parser;

import com.example.jrd48.service.MyLogger;

/**
 * Created by Administrator on 2016/8/29 0029.
 */

public class Crc16 {
    private static int[] auchCRCHi = {
            };
    private static int[] auchCRCLo = {
            };

    /**
     * 判断检验字
     *
     * @param buff  要判断的Buff，
     * @param len   要判断的长度，不能大于Buff的长度
     * @param expectedValue
     * @return true or false
     * @throws ParseException len 错误
     */
    public static boolean equalCrc(byte[] buff, int len, int expectedValue) throws ParseException {
        int calcValue = calcCrc(buff, len);
        boolean bOk = ( calcValue == expectedValue);
        if (!bOk) {
            MyLogger.jLog().w("crc failed: expected: "+ Integer.toHexString(expectedValue)+", but got: "+ Integer.toHexString(calcValue));
        }
        return bOk;
    }

    /**
     * 计算整个Buffer的检验字
     *
     * @param puchMsg
     * @return
     * @throws ParseException
     */
    public static int calcCrc(byte[] puchMsg) throws ParseException {
        return calcCrc(puchMsg, puchMsg.length);
    }

    /**
     * 计算检验字
     *
     * @param puchMsg 要计算的Buff
     * @param len     要计算的长度（字节数），不能超过Buff的长度
     * @return 两字节的 Int
     * @throws ParseException
     */
    public static int calcCrc(byte[] puchMsg, int len) throws ParseException {
        if (puchMsg.length < len) {
            throw new ParseException("RTU_CRC len parameter error.");
        }

        int usDataLen = len;

        int uchCRCHi;   //highbyteofCRCinitialized
        int uchCRCLo;   //lowbyteofCRCinitialized
        int uIndex;     //willindexintoCRClookuptable
        uchCRCHi = 0xFF;
        uchCRCLo = 0xFF;

        for (int i = 0; i < usDataLen; ++i) {
            uIndex = uchCRCHi ^ (puchMsg[i]);
            uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex & 0xff];
            uchCRCLo = auchCRCLo[uIndex & 0xff];
        }
        return (uchCRCHi << 8 | uchCRCLo);
    }


}
