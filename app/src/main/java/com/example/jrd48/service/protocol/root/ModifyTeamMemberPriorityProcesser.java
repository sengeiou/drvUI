...
public class ModifyTeamMemberPriorityProcesser extends CommonProcesser {
    public final static String ACTION = "ACTION.ModifyTeamMemberPriorityProcesser";

    public ModifyTeamMemberPriorityProcesser(Context context) {
        super(context);
    }
    @Override
    public void onGot(byte[] data) {
        Log.i("chat", "获得修改群组成员优先级应答: ");
        try {
            ProtoMessage.CommonResp resp = ProtoMessage.CommonResp.parseFrom(ArrayUtils.subarray(data, 4, data.length));
            Log.i("chat", "获得修改群组成员优先级校验码: " + resp.getErrorCode());
