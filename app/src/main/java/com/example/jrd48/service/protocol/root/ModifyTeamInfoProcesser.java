...
public class ModifyTeamInfoProcesser extends CommonProcesser {
    public final static String ACTION = "ACTION.ModifyTeamInfoProcesser";

    public ModifyTeamInfoProcesser(Context context) {
        super(context);
    }
    @Override
    public void onGot(byte[] data) {
        Log.i("chat", "获得修改群组信息应答: ");
        try {
            ProtoMessage.CommonResp resp = ProtoMessage.CommonResp.parseFrom(ArrayUtils.subarray(data, 4, data.length));
            Log.i("chat", "修改群组信息校验码: " + resp.getErrorCode());
