...
//            resCoreParameters.mediacodecAVCFrameRate = resCoreParameters.videoFPS;
            loopingInterval = 1000 / resConfig.getVideoFPS();
            dstVideoFormat = new MediaFormat();
            synchronized (syncDstVideoEncoder) {
                dstVideoEncoder = MediaCodecHelper.createSoftVideoMediaCodec(dstVideoFormat);
            ...
            //video
            videoWidth = resConfig.VIDEO_WIDTH;
            videoHeight = resConfig.VIDEO_HEIGHT;
            videoQueueNum = resConfig.getVideoBufferQueueNum();
            previewColorFormat = ImageFormat.YV12;
            int orignVideoBuffSize = BuffSizeCalculator.calculator(videoWidth, videoHeight, previewColorFormat);
            orignVideoBuffs = new RESVideoBuff[videoQueueNum];
            for (int i = 0; i < videoQueueNum; i++) {
                orignVideoBuffs[i] = new RESVideoBuff(previewColorFormat, orignVideoBuffSize);
            }
            lastVideoQueueBuffIndex = 0;
//            resCoreParameters.previewBufferSize = orignVideoBuffSize;
            ...
            filteredNV21VideoBuff = new RESVideoBuff(MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar,
                    BuffSizeCalculator.calculator(videoWidth, videoHeight, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar));
            int mediacodecAVCColorFormat = dstVideoFormat.getInteger(MediaFormat.KEY_COLOR_FORMAT);
            suitable4VideoEncoderBuff = new RESVideoBuff(mediacodecAVCColorFormat,
                    BuffSizeCalculator.calculator(videoWidth, videoHeight, mediacodecAVCColorFormat));
...
//            loopingInterval = 1000 / resCoreParameters.videoFPS;
    ...
    public void startPreview(SurfaceTexture surfaceTexture, int visualWidth, int visualHeight) {
        synchronized (syncPreview) {
            ...
            switch (RESConfig.obtain().getRenderingMode()) {
    ...
    public void queueVideo(byte[] rawVideoFrame) {
        synchronized (syncOp) {
            ...
            if (orignVideoBuffs[targetIndex].isReadyToFill) {
                ...
                acceptVideo(rawVideoFrame, orignVideoBuffs[targetIndex].buff,0,0);
    ...
    public void queueVideo(byte[] rawVideoFrame,int videoWidth,int videoHeight) {
        synchronized (syncOp) {
            int targetIndex = (lastVideoQueueBuffIndex + 1) % orignVideoBuffs.length;
            if (orignVideoBuffs[targetIndex].isReadyToFill) {
                LogTools.d("queueVideo,accept ,targetIndex" + targetIndex);
                acceptVideo(rawVideoFrame, orignVideoBuffs[targetIndex].buff,videoWidth,videoHeight);
                orignVideoBuffs[targetIndex].isReadyToFill = false;
                lastVideoQueueBuffIndex = targetIndex;
                videoFilterHandler.sendMessage(videoFilterHandler.obtainMessage(VideoFilterHandler.WHAT_INCOMING_BUFF, targetIndex, 0));
            } else {
    ...
    private void acceptVideo(byte[] src, byte[] dst,int videoWidth,int videoHeight) {
        int directionFlag = RESClient.getInstance().getCoreParameters().frontCameraDirectionMode;
        ColorHelper.NV21Transform(src,
                dst,
                videoWidth,
                videoHeight,directionFlag);
    }
    ...
    public void setVideoFilter(BaseSoftVideoFilter baseSoftVideoFilter) {
        ...
        if (videoFilter != null) {
            videoFilter.onInit(videoWidth, videoHeight);
        }
        lockVideoFilter.unlock();
    }
            ...
            switch (msg.what) {
                ...
                case WHAT_DRAW: {
                    ...
                    if (isFilterLocked) {
                         ...
                         */
                        if (dstVideoFormat.getInteger(MediaFormat.KEY_COLOR_FORMAT) == MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar) {
                            ColorHelper.NV21TOYUV420SP(modified ? filteredNV21VideoBuff.buff : orignNV21VideoBuff.buff,
                                    suitable4VideoEncoderBuff.buff, videoWidth * videoHeight);
                        } else if (dstVideoFormat.getInteger(MediaFormat.KEY_COLOR_FORMAT) == MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420Planar) {
        ...
        private void checkScreenShot(byte[] pixel) {
            synchronized (syncResScreenShotListener) {
                if (resScreenShotListener != null) {
                    int[] argbPixel = new int[videoWidth * videoHeight];
